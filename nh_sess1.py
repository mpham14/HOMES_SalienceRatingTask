...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/mettapham/Downloads/Nursing Home Task Files/nh_sess1.py',
...
imageLandmarks = visual.ImageStim(
    win=win,
    name='imageLandmarks', 
...
landmarkName_text = visual.TextStim(win=win, name='landmarkName_text',
    ...
    languageStyle='LTR',
    depth=0.0);
landmarkName_resp = keyboard.Keyboard()
...
for thisTrial in trials:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    imageLandmarks.setImage(landmark_images)
    # keep track of which components have finished
    trialComponents = [imageLandmarks]
    ...
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        # *imageLandmarks* updates
        if imageLandmarks.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            imageLandmarks.frameNStart = frameN  # exact frame index
            imageLandmarks.tStart = t  # local t and not account for scr refresh
            imageLandmarks.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(imageLandmarks, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'imageLandmarks.started')
            imageLandmarks.setAutoDraw(True)
        if imageLandmarks.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > imageLandmarks.tStartRefresh + 5-frameTolerance:
                # keep track of stop time/frame for later
                imageLandmarks.tStop = t  # not accounting for scr refresh
                imageLandmarks.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'imageLandmarks.stopped')
                imageLandmarks.setAutoDraw(False)
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    salience_text.setText('On a scale of 1 to 5, how salience would you rate the object?')
    salience_resp.keys = []
    salience_resp.rt = []
    _salience_resp_allKeys = []
    # keep track of which components have finished
    feedback_salienceComponents = [salience_text, salience_resp]
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    visualcomplexity_text.setText('On a scale of 1 to 5, how visually complex was the object?')
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    landmarkName_text.setText('What would you name this object?\n')
    landmarkName_resp.keys = []
    landmarkName_resp.rt = []
    _landmarkName_resp_allKeys = []
    feedbackText.reset()
    feedbackText.setText(feedbackText)
    # Run 'Begin Routine' code from codeNameGeneration
    feedbackText = ''
    ...
    key_list = []
    # keep track of which components have finished
    name_generationComponents = [landmarkName_text, landmarkName_resp, feedbackText]
    ...
    while continueRoutine:
        ...
        # *landmarkName_resp* updates
        waitOnFlip = False
        if landmarkName_resp.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            landmarkName_resp.frameNStart = frameN  # exact frame index
            landmarkName_resp.tStart = t  # local t and not account for scr refresh
            landmarkName_resp.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(landmarkName_resp, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'landmarkName_resp.started')
            landmarkName_resp.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(landmarkName_resp.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(landmarkName_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if landmarkName_resp.status == STARTED and not waitOnFlip:
            theseKeys = landmarkName_resp.getKeys(keyList=['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','space','backspace','return'], waitRelease=False)
            _landmarkName_resp_allKeys.extend(theseKeys)
            if len(_landmarkName_resp_allKeys):
                landmarkName_resp.keys = [key.name for key in _landmarkName_resp_allKeys]  # storing all keys
                landmarkName_resp.rt = [key.rt for key in _landmarkName_resp_allKeys]
        ...
        # Run 'Each Frame' code from codeNameGeneration
        #if a new key has been pressed since last time
        if(len(landmarkName_resp.keys) > last_len):
        
            
        
            #increment the key logger length
            last_len = len(landmarkName_resp.keys)
        
            
        
            #grab the last key added to the keys list
            key_list.append(landmarkName_resp.keys.pop())
    ...
    for thisComponent in name_generationComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if landmarkName_resp.keys in ['', [], None]:  # No response was made
        landmarkName_resp.keys = None
    trials.addData('landmarkName_resp.keys',landmarkName_resp.keys)
    if landmarkName_resp.keys != None:  # we had a response
        trials.addData('landmarkName_resp.rt', landmarkName_resp.rt)
    # Run 'End Routine' code from codeNameGeneration
    thisExp.addData('nameFeedback', feedbackText)
